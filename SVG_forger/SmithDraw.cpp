...
void Smith::drawCrossword()
{
	...
	double fromTop = 1.4, fromSide = 0.85;
	...
	coef = (14.0*arrowShrinkRay) / currArrow.getWidth();
			...
			for (int c = 0; c < sq.clues.size(); c++) {
				...
				if (i > currWord.i && j == currWord.j) {
					if (currWord.hor) { // top down 0
						...
						string color = "purple";
						textGroup.add_copy(ClueToSVG(sq.clues[c], "start", j * 14 - squareOutW + fromSide, i * 14 - squareOutH + fromTop + letterH, color,currWord.actualWord));
						sq.topOccupied = 1;
					}
					else { // Error
						auto arrowPath = SVG_element("path");
						...
						arrowGroup.add_copy(arrowPath);
						string color = "purple";
						textGroup.add_copy(ClueToSVG(sq.clues[c], "middle", (j+0.5)*14 - squareOutW + fromSide, i * 14 - squareOutH + fromTop + letterH, color, currWord.actualWord));
						sq.topOccupied = 1;
					}
				}
			}
			for (int c = 0; c < sq.clues.size(); c++) {
				...
				if (j < currWord.j) {
					if (currWord.hor) { // right up 0
						...
						if (!sq.topOccupied)textGroup.add_copy(ClueToSVG(sq.clues[c], "end", (j + 1) * 14 + squareOutW - fromSide, i * 14 - squareOutH + fromTop + letterH + (allowedSpace > 0 ? letterH / 2 : 0), color, currWord.actualWord));
						...
						if (!sq.topOccupied)textGroup.add_copy(ClueToSVG(sq.clues[c], "end", (j + 1) * 14 + squareOutW - fromSide, i * 14 - squareOutH + fromTop + letterH, color, currWord.actualWord));
						else textGroup.add_copy(ClueToSVG(sq.clues[c], "end", (j + 1) * 14 + squareOutW - fromSide, (i+1) * 14 + squareOutH - fromTop - letterH * (currWord.clue[sq.word_clue_ind[c].second].second - 1), color, currWord.actualWord));
						sq.topOccupied = 1;
					}
				}
			...
			for (int c = 0; c < sq.clues.size(); c++) {
				...
				if (j > currWord.j && i == currWord.i) {
					if (currWord.hor) { // Error
						...
						if (!sq.topOccupied)textGroup.add_copy(ClueToSVG(sq.clues[c], "start", j * 14 - squareOutW + fromSide, i * 14 - squareOutH + fromTop + letterH, color, currWord.actualWord));
	...
	for (auto &i : forHelpDict) {
		if (alphaDict.count(clearString(i)) && helpDictBr < 10)putInDictString.push_back(clearString(i)), helpDictBr++;
	}
	for (auto &i : putInDictString) {
		helpDict += i + ", ";
	}
	if (helpDictBr)helpDict.pop_back(),helpDict.pop_back();

	auto dictionaryRectText = ClueToSVG(helpDict, "start", 0 + 1 + dicStroke, 14 * cross.N + 3.1, dictColor, "dict");
	...
	auto dictionaryAuthor = ClueToSVG(author, "end", 14*cross.M - 1 - dicStroke, 14 * cross.N + 3.1, dictColor, "author");
	...
	if (!(crossFileName[0] == 'k' && crossFileName[1] >= '0' && crossFileName[1] <= '9')) {
		...
		dictionaryAuthor["y"] = to_string(14 * cross.N + 5.5);
		dictionaryAuthor["x"] = to_string(14 * cross.M);
		dictionaryRectText["y"] = to_string(14 * cross.N + 5.5);
		dictionaryRectText["x"] = to_string(0);
	}

	SVG_element answers("g");
	auto ansBorder = SVG::GetBox(0, 0, cross.M * 7, cross.N * 7, 0.5);
